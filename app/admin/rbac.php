...
function int_array_from_post(string $key): array {
    ...
    $out = [];
    foreach ($raw as $v) {
        if (is_string($v) && ctype_digit($v)) $out[] = (int)$v;
        if (is_int($v) && $v > 0) $out[] = $v;
    }
...
function refresh_auth_session(PDO $pdo, int $userId): void {
    if (!function_exists('load_roles_to_session') || !function_exists('load_permissions_to_session')) return;
    ...
    // bywa string
    if (!(is_int($sid) || (is_string($sid) && ctype_digit($sid)))) return;
    if ((int)$sid !== $userId) return;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'set_user_role') {
            ...
            if ($isAjax) ajax_json(['ok' => true, 'msg' => 'Zapisano rolę użytkownika.']);
            exit;
        }
        ...
        if ($action === 'save_role_perms') {
            ...
            if (isset($_SESSION['user_id']) && (is_int($_SESSION['user_id']) || ctype_digit((string)$_SESSION['user_id']))) {
                ...
                if (function_exists('load_roles_to_session')) load_roles_to_session($pdo, $sid);
                if (function_exists('load_permissions_to_session')) load_permissions_to_session($pdo, $sid);
            }

            if ($isAjax) ajax_json(['ok' => true, 'msg' => 'Zapisano uprawnienia roli.']);
            exit;
        }
        ...
        if ($isAjax) ajax_json(['ok' => false, 'err' => $e->getMessage()], 400);
...
<style>
.rbac-wrap { max-width: 980px; }
.rbac-tabs a { margin-right: 10px; }
.rbac-toast { margin: 10px 0; padding: 8px 10px; border-radius: 6px; display:none; }
.rbac-toast.ok { display:block; border:1px solid rgba(0,128,0,.35); }
.rbac-toast.err { display:block; border:1px solid rgba(255,0,0,.35); }
.rbac-table { width: 100%; border-collapse: collapse; }
.rbac-table th, .rbac-table td { padding: 10px 8px; border-bottom: 1px solid rgba(255,255,255,.08); text-align:left; }
.rbac-small { opacity: .7; font-size: 12px; }
.rbac-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
@media (max-width: 820px) { .rbac-grid { grid-template-columns: 1fr; } }
</style>
